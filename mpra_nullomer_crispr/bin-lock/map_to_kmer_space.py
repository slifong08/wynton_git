...
Example 
    Say we find an 11mer, GCGTACGTACG, that appears 15025 times on chrN. The key length is 4. 
        ...
        The 11mer coordinates for GCGTACGTACG, will be written to GCGT.csv

        The resulting file will look like this:

        ./GCGT.csv
            ACGTACG, chr1:400,000-400,500
Limitations
...
arg_parser.add_argument("cellline", type=str, help='cellline')
...
FASTA, CL,  WINDOW_SIZE, KEYSIZE, CONFIG = args.fasta, args.cellline, args.length, args.keysize, args.config
...
def makeOutDir(path, windowsize):
    OUTDIR = os.path.join(path, "kmers", f"{windowsize}mers", "coor")
...
def getOneSeqKmers(windowsize, sequence, seq_id):
    ...
    inputs
        ...
        seq_id (str) - coordinates the sequence belongs to
...
def writeDictionary(name, outpath, windowsize, result_dict): # keyset
    ...
    for key, value in result_dict.items():
        ...
        out_file = os.path.join(outpath, f"{key}_coor.csv")
...
def main(argv):
    ...
    OUTDIR = makeOutDir(PATH, WINDOW_SIZE)
    
    config, cfn = crw.read(CONFIG)
    section = f"{CL}.{WINDOW_SIZE}mer"
    crw.check(config, section)
    config[section]["coor"]= OUTDIR
    ...
    result_multi = [partial_seq( sequence, seq_id) for seq_id, sequence in SEQ.items()]
    result_multi_r = [partial_rev(sequence, seq_id) for seq_id, sequence in REV.items()]
