...
def getSummaryStats(file, value_col):
    ...
    if os.path.exists(out) is False:
        ...
        summary_stats["index"] = summary_stats["index"] - 4 # correct bp position 
...
def main(argv):
    ...
    1. extracts the phylop base scores for the expanded locus, writing the scores as a vector
    2. write summary stats of file. (Summary stats at each bp)
    ...
    if os.path.exists(outfile +".gz") is False and os.path.exists(outfile) is False:
        ...
        #1 get phylop values from bigwig and write to the outfile. 
        with open(BED, "r") as ex:

            print("reading expanded file", BED)
    ...
    # 2
    summary_outfile = getSummaryStats(outfile, "phylop")
