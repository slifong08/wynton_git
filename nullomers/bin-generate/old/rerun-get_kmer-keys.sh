...
#$ -l scratch=64G      # job requires up to 2 GiB of local /scratch space
#$ -l h_rt=02:00:00   # job requires up to 24 hours of runtime
##$ -t 1-10           # array job with 10 tasks (remove first '#' to enable)
#$ -r y               # if job crashes, it should be restarted
#$ -m be              # alerts to mail about
...
##$ -t 1-24:1        # job range:step size
##$ -tc 24             # n jobs to run at once
...
python3 /wynton/home/ahituv/fongsl/nullomers/bin-generate/get_kmer-keys.py "$1" "$2" "$3" "$4"  # ORDER OF ARGS MATTERS
# $SGE_TASK_ID = chrnum
# $1 = output directory
# $2 = kmer length
# $3 = key length
