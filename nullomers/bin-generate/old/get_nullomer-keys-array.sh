...
#$ -l mem_free=200G     # job requires up to 1 GiB of RAM per slot
#$ -l scratch=200G      # job requires up to 2 GiB of local /scratch space
...
##$ -t 1-1003:1        # job range:step size
##$ -tc 25             # n jobs to run at once
...
if (( kmer_length > 16 )); then
    echo "concatenating kmers only"
    python3 /wynton/home/ahituv/fongsl/nullomers/bin-generate/cat_kmer.py $SGE_TASK_ID "$1" "$2" "$3" "$4" # ORDER OF 
else
    echo "making kmers and nullomers"
    python3 /wynton/home/ahituv/fongsl/nullomers/bin-generate/get_nullomer-keys-array.py $SGE_TASK_ID "$1" "$2" "$3" "$4" # ORDER OF ARGS MATTERS
