...
def main(argv):
    ...
    #1
    if CL =="common":
        KMER_REF = f"/wynton/home/ahituv/fongsl/MPRA/agarwal_2023/kmers/keys/{MER}mers/*.csv.gz"
    else:
        ...
        for seq, seq_count in seq_dict.items():  #seq = "ACCGGTTCCACGGT"
            ...
            if len(null_list) > 0:
                ...
                for seq_list, datatype in infos:
                    ...
                    # write fa
                    farow = writeFa(seq, seq_list, seq_id, datatype)
                    fwriter.write(farow)

                    # write information to tsv file.
                    rows = makeNullrow(seq, seq_count, seq_list, NMUTS, seq_id, datatype)
