...
process 'create_BWA_ref' {
    ...
    conda 'conf/mpraflow_wynton.yml'
...
if (params.fastq_insertPE_file != null) {
	...
	conda 'conf/mpraflow_wynton.yml'
        ...
        shell:
            """
            /wynton/group/ahituv/bin/bwa mem $design $chunk | \
            /wynton/group/ahituv/bin/samtools sort - -o ${name}.${chunk}.sorted.bam

            echo 'bam made'

            #/wynton/group/ahituv/bin/samtools view ${name}.${chunk}.sorted.bam | head

            /wynton/group/ahituv/bin/samtools view ${name}.${chunk}.sorted.bam | \
	...
	conda 'conf/mpraflow_wynton.yml'
        ...
        shell:
            """
            /wynton/group/ahituv/bin/bwa mem $design $chunk | /wynton/group/ahituv/bin/samtools sort - -o ${name}.${chunk}.sorted.bam
            echo 'bam made'
            /wynton/group/ahituv/bin/samtools view ${name}.${chunk}.sorted.bam | head
            /wynton/group/ahituv/bin/samtools view ${name}.${chunk}.sorted.bam | wc -l > ${chunk}_count_bam.txt
            """
    }
}
...
process 'collect_chunks'{
    ...
    shell:
        ...
        /wynton/group/ahituv/bin/samtools merge all.bam $sbam_list
        /wynton/group/ahituv/bin/samtools sort all.bam -o s_merged.bam

        #collect bam counts into one file

        /wynton/group/ahituv/bin/samtools view s_merged.bam | wc -l > count_merged.txt
        """
}
