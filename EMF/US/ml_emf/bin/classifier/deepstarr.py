...
parser.add_argument('pred_task', type=str)  
parser.add_argument("-s", '--standard_scaling', type=bool, default=False)  
...
def prepare_input(set, prefix, scale=False):
    ...
    # added to encode labels
    if params["pred_task"]=="class":
        print("\n\n label encode Y\n\n")
        # encode class labels for each array
        for i in np.arange(len(Y)):
            Y[i] = labelEncoder(np.array(Y[i]))
...
def DeepSTARR(col_names, params=params):
    ...
    if pred_task == "reg":
        ...
        if params['n_pred_tasks'] == 1:
            activation_ = "sigmoid"  # use 'softmax' if binary #tf.nn.sigmoid
        else:
            activation_ = "softmax"  # use 'softmax' if multiclass #tf.nn.softmax
...
def summary_statistics(X, Y, set_var, params=params):
    ...
    if main_params['pred_task'] == "reg":
        for i, task in enumerate(list(Y)):
        ...
        # report the accuracy    
        print(set_var, "accuracy " + " = " + str("{0:0.2f}".format(pred_acc)))
...
#update dictionary with actual N pred tasks based on col name
params["n_pred_tasks"] = len(col_names)  
...
train_pred = summary_statistics(X_train, Y_train, "train")
val_pred = summary_statistics(X_valid, Y_valid, "validation")
test_pred = summary_statistics(X_test, Y_test, "test")
...
if "class" in prefix:
    ...
    sns.boxplot(x=list(preds)[0], y=list(preds)[1], data=preds.round(2))
    ax.set(xlabel="test", 
          ylabel="pred_class")
    plt.savefig('model_test.png', bbox_inches='tight')

    # write descriptive stats
    preds.groupby("Y").describe().T.to_csv("model_test_stats.csv")
