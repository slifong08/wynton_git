...
class SeqNN(nn.Module):
    ...
    def __init__(self, 
        ...
        # architecture of block (Stem block)
        block = nn.Sequential(
                       nn.Conv1d(
        ...
        for ind, (prev_sz, sz) in enumerate(zip(block_sizes[:-1], block_sizes[1:])):
            block = nn.Sequential(
                       ...
                       # squeeze and excite layer of the block
        ...
        self.seqextractor = nn.ModuleDict(seqextblocks)  # extract sequences?
    ...
    def forward(self, x, predict_score=True):    
        ...
        x = x.squeeze(2)
        logprobs = F.log_softmax(x, dim=1)  # compute the log prob
        if predict_score:
            x = F.softmax(x, dim=1)
...
# add new layer to beginning of model: https://discuss.pytorch.org/t/add-a-layer-to-the-beginning-of-a-trained-model/7220/2        
class TLModel(nn.Module):
    def __init__(self, 
                ...
                se_reduction=4,
                final_ch=18,
                bn_momentum=0.1):  # added needed params
        ...
        # add a new layer to top of the model, to adjust model for size of input sequences
        self.new_firstlayer = nn.Sequential(
                             nn.Conv1d(
                                in_channels=6 if self.use_single_channel else 5, 
                                out_channels=6 if self.use_single_channel else 5, # return the same number of channels
                                kernel_size=self.ks, # changed - was ks
                                padding='same',
                                bias=False
                           ),
        ...
        self.new_lastlayer = nn.Conv1d(
                            in_channels=self.final_ch,
                            out_channels=self.final_ch,
                            kernel_size=1,
                            padding='same',
                       )
    ...
    def forward(self, inputs):
        ...
        x = self.new_firstlayer(inputs) 
        
        return self.pretrained_model(x)
        #return self.new_lastlayer(x)
