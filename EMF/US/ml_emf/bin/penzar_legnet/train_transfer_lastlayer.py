...
parser.add_argument("--model", type=str, default="/wynton/home/ahituv/fongsl/EMF/US/ml_emf/data/legnet/model_300.pth", help="path to a .pth file where parameters are stored")
parser.add_argument("--tl_layers", action='store_true', help="add new input/output layers")
parser.add_argument("--freeze_layers", action='store_true', help="freeze original network layers")
parser.add_argument("--unfrozen_layers", type=int, default=0, help="number of layers to unfreeze in original network layers")
...
class PearsonMetric(Metric):
    ...
    def compute(self):
        ...
        y = np.nan_to_num(y)  # added to fill na, 
        y_pred = np.nan_to_num(y_pred) # added to fill na, 
...
def create_trainer(model, 
    ...
    def train_step(trainer, batch):
        ...
        #loss = Variable(loss, requires_grad = True)  # changed - added because of this error - https://stackoverflow.com/questions/62699306/runtime-error-element-0-of-tensors-does-not-require-grad-and-does-not-have-a-g
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        scheduler.step()                                                                                         
    ...
    # write validation scores to .json file per epoch. 
    def validate(engine):
        p.reset()
...
print(args)
print("\nadd new transfer learning layers", args.tl_layers)
...
os.system(f"cp {args.model} {args.model_dir}")  # changed - copy model into model_dir
...
if not os.path.exists(args.temp):  # changed using os.path.exists
    args.temp.mkdir()

# make training and validation files
FROM = os.path.split(args.train_valid_path)[1] # changed path split strategy, made FROM var
...
#print("SUMMARY", summary(model)) # added
...
# add new input/output layers?

if bool(args.tl_layers) is True:
    # from https://discuss.pytorch.org/t/add-a-layer-to-the-beginning-of-a-trained-model/7220/4
    
    # added class to model.py
    model = TLModel(model, seqsize=args.seqsize, use_single_channel=args.use_single_channel, block_sizes=args.blocks, ks=args.ks,
              resize_factor=args.resize_factor, se_reduction=args.se_reduction, final_ch=args.final_ch)
    print("model after adding new first/last layer", model)

else:
    print("running out-of-box model")
    
# Freeze layers? 
if args.freeze_layers is True:
    
    nlayers = len(list(model.parameters()))
    
    # If args.unfrozen_layers = zero, all layers frozen
    freeze_layers = abs(args.unfrozen_layers - nlayers)  # get the number of layers that should remain frozen. 
    
    print(f"freezing all {freeze_layers} of {nlayers} layers, except the first and last layer")
    
    current_layer=0
    for name, param in model.named_parameters():
        #print(name)
        current_layer+=1  # keep count of which layer you are on
        
        # make sure to freeze only some layers
        if("layer" not in name) and current_layer<freeze_layers:
